...
export default async function Image() {
  ...
  const isLogoUriLocal = logoUri.includes('lk-logo') || logoUri.includes('hail-ride-logo');
  const isDefaultLogo =
    logoUri === APP_CONFIG_DEFAULTS.logoDark || logoUri === APP_CONFIG_DEFAULTS.logo;
  const wordmarkUri = isDefaultLogo ? 'public/hail-ride-wordmark.svg' : logoUri;
  ...
  const { base64: logoSrcBase64, dimensions: logoDimensions } = await getImageData(
    logoUri,
    'public/hail-ride-logo-dark.svg'
  );
